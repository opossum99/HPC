...
int main(int argc, char** argv) {
	...
	int number_of_sizes = 13;

	std::vector<int> sizes(number_of_sizes);
	...
	for(int i = 0; i < number_of_sizes; i++)
	{
		...
		int N = 1000000; //std::atoi(argv[1]); //Number of iterations.

		int size_of_message = sizes[i];//std::atoi(argv[2]); // It should be greater than 1.
...
// 			MyFile.open("table.csv", std::ios::app);
			...
			Time << size_in_bytes << " " << duration << std::endl;
			Bandwidth << size_in_bytes << " " << memory << std::endl;
